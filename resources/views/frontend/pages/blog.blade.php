    ...
    <div class="container d-flex flex-column gap-md">
        ...
        <div class="blog-filter">
            <div class="blog-filter-item {{ request('category') ? '' : 'active' }}">
                <a href="{{ app()->getLocale() == 'ar' ? route('blog.index') : route('blog.index.en') }}"
                   class="sub-heading-5 text-body mb-0">{{ __('site.all') }}</a>
            </div>
            @foreach($categories as $cat)
                @php
                    $name = app()->getLocale() === 'ar' ? $cat->name_ar : $cat->name_en;
                @endphp
                <div class="blog-filter-item {{ request('category') == $cat->id ? 'active' : '' }}">
                    <a href="{{ app()->getLocale() == 'ar'
                                ? route('blog.index', ['category' => $cat->id])
                                : route('blog.index.en', ['category' => $cat->id]) }}"
                       class="sub-heading-5 text-body mb-0">
                        {{ $name }}
                    </a>
                </div>
            @endforeach
