...
@section('title', 'تعديل الحملة - ' . $campaign->name)
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="mb-1">تعديل الحملة: {{ $campaign->name }}</h2>
    ...
    <div class="card shadow">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0"><i class="fas fa-edit me-2"></i>تعديل معلومات الحملة</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.marketing.campaigns.update', $campaign) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('type') is-invalid @enderror" 
                                ...
                                <option value="email" {{ old('type', $campaign->type) == 'email' ? 'selected' : '' }}>بريد إلكتروني</option>
                                <option value="sms" {{ old('type', $campaign->type) == 'sms' ? 'selected' : '' }}>رسائل نصية</option>
                                <option value="social" {{ old('type', $campaign->type) == 'social' ? 'selected' : '' }}>وسائل التواصل الاجتماعي</option>
                                <option value="banner" {{ old('type', $campaign->type) == 'banner' ? 'selected' : '' }}>إعلانات بانر</option>
                                <option value="popup" {{ old('type', $campaign->type) == 'popup' ? 'selected' : '' }}>نوافذ منبثقة</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('target_audience') is-invalid @enderror" 
                                   id="target_audience" name="target_audience" value="{{ old('target_audience', $campaign->target_audience) }}"
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('budget') is-invalid @enderror" 
                                   id="budget" name="budget" value="{{ old('budget', $campaign->budget) }}" step="0.01" min="0">
                            @error('budget')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('start_date') is-invalid @enderror" 
                                   id="start_date" name="start_date" value="{{ old('start_date', $campaign->start_date ? $campaign->start_date->format('Y-m-d') : '') }}" required>
                            @error('start_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('end_date') is-invalid @enderror" 
                                   id="end_date" name="end_date" value="{{ old('end_date', $campaign->end_date ? $campaign->end_date->format('Y-m-d') : '') }}" required>
                            @error('end_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-12">
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('goals') is-invalid @enderror" 
                                      ...
                                      placeholder="مثال: زيادة المبيعات بنسبة 20%، جذب 100 عميل جديد، إلخ">{{ old('goals', $campaign->goals) }}</textarea>
                            @error('goals')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-12">
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('content') is-invalid @enderror" 
                                      id="content" name="content" rows="5">{{ old('content', $campaign->content) }}</textarea>
                            @error('content')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
